            ...
            if i >= 2 * self.pivot_point_period {
                ...
                if let Some(center) = self.center_line.get() {
                    ...
                    if atr > 0.0 {
                        ...
                        self.candles_trend.insert(
                            candles[i].timestamp,
                            self.supertrend_state
                                .as_ref()
                                .expect("there should always be a trend after the first iteration")
                                .trend,
                        ); // Store trend for each candle (for potential later use in outcomes matrix)
                    }
                }
            }
        }
...
mod tests {
    ...
    fn test_ppst_initialization() {
        let ppst = PPST::new(10, 3.0, 14, 100, AllowedTimeframe::M1);
        ...
        assert_eq!(ppst.timeframe, AllowedTimeframe::M1);
